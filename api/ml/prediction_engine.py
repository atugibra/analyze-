"""
Prediction Engine for FootballIQ
==================================
Singleton-style engine that:
  1. Trains on ALL historical matches from Supabase
...
def train_model():
    """
    Pull all completed matches from Supabase, build 70+ feature vectors,
    train the ensemble, save to disk.
    ...
    try:
        ...
        # Get feature names via a single build (not expensive)
...
def _key_factors(home_feats: dict, away_feats: dict,
    ...
    """
    Generate human-readable key factors explaining the prediction, by
    comparing the most impactful feature pairs.
    """
    factors = []
    ...
    # Scoring Patterns (Blanks & Blowouts)
    ...
    return factors[:6]  # top 6 only


def predict_match(home_team_id: int, away_team_id: int,
                  league_id: int, season_id: int) -> dict:
    """
    Full rich prediction for one match. Returns comprehensive output dict.
    """
    engine = _get_engine()
...
def predict_upcoming(league_id: int = None, limit: int = 50) -> list:
    ...
    Returns list of rich prediction dicts.
    """
    engine = _get_engine()
    if engine is None or not engine.is_trained:
        return []
...
def get_status() -> dict:
    ...
    return {
        ...
        "feature_names":   _meta.get("feature_names", [])[:20],  # first 20 for inspection
    }
