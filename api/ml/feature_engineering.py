"""
Feature Engineering for FootballIQ Prediction Engine
=====================================================
Extracts 80+ features per team from ALL Supabase tables:
...
def compute_h2h(cur, home_team_id, away_team_id, n=10):
    ...
    for r in rows:
        # Always read from perspective of home_team_id
...
def build_player_features(cur, team_id, season_id):
    ...
    top             = players[0]  # sorted by goals DESC
    ...
    # Top scorer per90 (proxy for elite finishing quality)
    ...
    # Team total assists per game (creativity from players table)
    cur.execute("""
...
def build_prev_season_features(cur, team_id, league_id):
    ...
    SCORES & FIXTURES / standings data already in Supabase.
    ...
    # Find the most recent season for this team in standings (other than 'current')
    cur.execute("""
    ...
    return {
        ...
        "prev_season_found": 1.0,   # signal that prior season data exists
    }
...
def build_prev_season_form(cur, team_id):
    ...
    """
    # The second-most recent season_id this team has played in
    cur.execute("""
        SELECT season_id
        FROM (
...
def build_team_features(cur, team_id, league_id, season_id, league_avgs=None):
    """
    Build features for a single team from league standings, squad stats, recent form, player stats, and previous season performance.
    ...
    if sq_ag:
        ...
        # Defensive shooting stats (opponent shots allowed)
    ...
    # ── 6. Previous season standings + match results ──────────────────────────
...
def build_match_features(cur, home_team_id, away_team_id, league_id, season_id):
    ...
    # Prefix home/away features and compute differentials
    vector = {}
    diff_keys = set()
